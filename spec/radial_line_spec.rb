describe "d3 - radial line" do
  it "d3.radial_line" do
    expect(CY.radial_line).to be_instance_of(CY::RadialLineGenerator)
  end

  let(:simple_data) { [[0,2],[Math::PI*0.10,5],[Math::PI*0.20,3]] }

  it "basics" do
    expect(CY.radial_line.(simple_data)).to eq("M0,-2L1.545084971874737,-4.755282581475767L1.7633557568774194,-2.4270509831248424")
  end

  it "curve" do
    expect(CY.radial_line.curve).to be_instance_of(CY::Curve)
    expect(CY.radial_line.curve(CY.curve_natural).(simple_data)).to eq(%W[
      M0,-2
  ...
  it ".angle constant" do
    radial_line = CY.radial_line.angle(Math::PI)
  ...
  it ".radius constant" do
    radial_line = CY.radial_line.radius(10)
  ...
  it ".defined" do
    radial_line = CY.radial_line.defined{|(a,r)| r.even?}
