...
x = CY.scale_linear.domain([0, StarTrekVoyager.size-1]).range([40, width-20])
y = CY.scale_linear.domain(StarTrekVoyager.map(&:rating).minmax).range([550, 50])
c = CY.scale_ordinal.range(CY.scheme_category_10)

(1..7).each do |season|
  episodes = StarTrekVoyager.select{|episode| episode.season == season }
  avg = CY.mean(episodes.map(&:rating))
  svg.append("line")
